...
build_windows:
	@GOOS=windows go build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/windows/amd64/securepush cmd/main.go  # windows

link_windows:
	@cp bin/windows/amd64/securepush ./bin/helmpush
...
build_linux:
	@GOOS=linux go build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/linux/amd64/securepush cmd/main.go  # linux

link_linux:
	@cp bin/linux/amd64/securepush ./bin/securepush
...
build_mac:
	@GOOS=darwin go build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/darwin/amd64/helmpush cmd/main.go # mac osx
	@cp bin/darwin/amd64/securepush ./bin # For use w make install

link_mac:
	@cp bin/darwin/amd64/securepush ./bin
